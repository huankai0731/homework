...
void Positioncontrol(geometry_msgs::Point &goal, turtlesim::Pose &follower, geometry_msgs::Twist &vel_msg, turtlesim::Pose &leader) {
	...
	if(abs(follower.theta-leader.theta)<0.1&&vel_msg.linear.x ==0){
		vel_msg.angular.z = 0;
	}
	
	//cout<<follower.theta-leader.theta<<endl;
}
...
int main(int argc, char **argv)
{
	...
	leadertoworld2D( follower2_goal, leader);
    	...
    	Positioncontrol(follower1_goal, follower1, follower1_twist, leader);
    	Positioncontrol(follower2_goal, follower2, follower2_twist, leader);
